...
;     always 4.  Integers can be negative: 1101,100,-1,4,0 is a valid program
...
(define TEST-diagnostic-program
  (map string->number (string-split (car (read-file "inputs/05.txt")) #\,)))

(intcode-compute TEST-diagnostic-program)
...
;     Opcode 7 is less than: if the first
;     parameter is less than the second parameter, it stores 1 in the position
